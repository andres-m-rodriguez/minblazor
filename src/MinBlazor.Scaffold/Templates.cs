...
internal static class Templates
{
    ...
    {
        var userPackages = string.Concat(packages.Select(p =>
            p.Version is null
                ? $"\n    <PackageReference Include=\"{p.Name}\" />"
                : $"\n    <PackageReference Include=\"{p.Name}\" Version=\"{p.Version}\" />"));

        var userProperties = string.Concat(properties
    ...
    {
        var consts = string.Concat(options
            .OrderBy(o => o.Key, StringComparer.Ordinal)
            .Select(o => $"    public const string {o.Key} = {SymbolDisplay.FormatLiteral(o.Value, true)};\n"));
